...
/*
Search queries the AUR for any packages matching the keyword. Searches can be
performed against any package detail described as a SearchType in this package.
...
/*
Info queries the AUR for detailed information about the requested packages. The
results will include details about licenses, package relationships, etc. If the
request generates a Go error, or the API returns an error, it is available in
err.
*/
func Info(packages []string) (results []aurPackage, err error) {
	...
	queryString := fmt.Sprintf("arg[]=%s", packages[0])
	if len(packages) > 1 {
		for _, pkg := range packages[1:] {
			queryString = fmt.Sprintf("%s&arg[]=%s", queryString, pkg)
		}
	}
...
/*
makeRequest handles the actual HTTP request to the AUR API. The query should
be an already-encoded string. Any strings passed as parts of the endpoint will
be appropriately joined. This function only checks for errors generated by Go
functions; callers are responsible for checking the AUR response for an error.
*/
